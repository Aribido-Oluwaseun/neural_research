...
class STAM:
    ...
    def __init__(self, num_train=10000, num_test=10000, num_valid=10000, data_type='cifar'):
        self.num_train = num_train
        self.num_test = num_test
        self.num_valid = num_valid
        self.data_type = data_type
        train_img, train_lbl, test_img, test_lbl = util.get_data(data_type=self.data_type, ZCA=True)
    ...
    def neural_algo(self, data,
        ...
        if W is None:
            ...
            elif W_init == 'NORM_INT':
                W = pd.DataFrame(data=np.array([np.random.normal(-0.42, 1, size=df.shape[1]-1)
        ...
        for T in df.index.tolist():
            ...
            if T < 5:
                ...
                plt.imshow(x_T.values.reshape(int(np.sqrt(len(x_T))), int(np.sqrt(len(x_T)))))
                plt.show()
...
class Experiments:
    ...
    def learn_w_with_set_clusters(self, data, lbl, num_clusters, use_saved_W=False, num_epochs=0,
        ...
        for i in range(num_epochs):
            if use_saved_W:
                ...
                W, matrix, total = self.stam_obj.run(data, lbl, num_clusters=num_clusters,
                                              get_mapping_matrix=get_mapping_matrix, order=order, W_init=W_init)
        print matrix
        print matrix.sum(axis=1)

        if save_matrix:
            name = order + '_expt1_' + W_init + '10000.xlsx'
            writer = pd.ExcelWriter(name)
            matrix.to_excel(writer, 'Sheet1')
            writer.save()
        ...
        for j in W.index.tolist():
            ...
            ax1.imshow(W.iloc[j, :].values.reshape(32, 32), interpolation='nearest', aspect='equal')
            ax1.axis('off')
        fig1.savefig('imgs/' + 'l1_nearest') #, dpi=150)

        fig1 = plt.figure()
        for j in W.index.tolist():
            ax1 = fig1.add_subplot(2, 5, j + 1)
            ax1.imshow(W.iloc[j, :].values.reshape(32, 32), interpolation='bilinear', aspect='equal')
            ax1.axis('off')
        fig1.savefig('imgs/' + 'l1_bilinear')

        # plt.bar(x=range(len(total)), height=total)
        # plt.title('weights in different clusters when number of neurons = {}'.format(num_clusters))
    ...
    def run_experiments(self):
        ...
        self.learn_w_with_set_clusters(train, train_lbl, num_clusters=num_clusters, use_saved_W=False,
                                       num_epochs=1, get_mapping_matrix=True, order='RANDOM_ORDER',
                                       W_init='NORM_INT')
