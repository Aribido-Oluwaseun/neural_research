...
class Hierarchy_STAMS:
    ...
    def intilialize_paramters(self):
        ...
        while i < self.layers:
            ...
            if (i != self.layers-1):
                ...
                n_li = int(np.sqrt(((self.rf_size[i]**2)*self.q[i])/(self.rf_size[i]/self.dim[i])**2))
            else:
                self.rf_size[i] = n_li-12
                self.q[i] = 1
                rf[i] = 1
                self.stride[i] = 1
            i += 1
        for i in range(self.layers):
            self.stams[i] = [Stam(num_clusters=self.neurons[i], stam_id=k, rf_size=self.rf_size[i],
                                      mapping_matrix=self.mapping_matrix[i]) for k in range(self.q[i])]
    ...
    def learn(self, image, lbl, layer, dim, red_dim=True):
        ...
        if red_dim:  # if are at the last layer, don't reduce dimension.
            ...
            images_dim = int(np.sqrt(len(self.weights[layer])))
...
def main():
    ...
    hierarchy_stams = Hierarchy_STAMS(data, lbl,  stride=[2,  1], layers=2, neurons=[20, 10], dim=[2, None], rf_size=[4, 8],
                 red_dim=(True, False), mapping_matrix=(True, False))
    hierarchy_stams.run(data, lbl)
    W = hierarchy_stams.get_weights(layers)
    for i in W.index.tolist():
        util.display_image(image=W.iloc[i, :], rows=14, cols=14)
